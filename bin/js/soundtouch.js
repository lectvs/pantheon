...
extend(WebAudioBufferSource.prototype, {
    get dualChannel() {
...
function PitchShifter(context, buffer, bufferSize, onEnd) {
    ...
    this._node = this.getWebAudioNode(
        context,
        this._filter,
        (sourcePostion) => this.onUpdate(sourcePostion),
        bufferSize
    );
    this.tempo = 1;
    this.rate = 1;
...
extend(PitchShifter.prototype, {
    get formattedDuration() {
        return this.minsSecs(this.duration);
    },

    get formattedTimePlayed() {
        return this.minsSecs(this.timePlayed);
    },
    ...
    get pitchSemitones() {
        return this._soundtouch.pitchSemitones;
    },
    ...
    onUpdate: function(sourcePosition) {
    ...
    minsSecs: function(secs) {
        const mins = Math.floor(secs / 60);
        const seconds = secs - mins * 60;
        return `${mins}:${this.pad(parseInt(seconds), 2)}`;
    },

    getWebAudioNode: function(context, filter, sourcePositionCallback, bufferSize) {
